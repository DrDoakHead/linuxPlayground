...
void Board::setSquares() 
{
    for (int8_t x = 0; x < 8; x++)
    {
        ...
        {
            std::shared_ptr<Square> square(new Square(Position(x, y)));
            board.at(x).push_back(square);
        }
    }
}
...
std::shared_ptr<Square> Board::getSquare(const Position& position) const
{
...
{
    std::shared_ptr<Square> initSquare = getSquare(startPosition);
    std::shared_ptr<Square> finalSquare = getSquare(finalPosition);
    makeMove(initSquare, finalSquare);
}

void Board::makeMove(std::shared_ptr<Square> initSquare, std::shared_ptr<Square> finalSquare)
{
...
void Board::capturePiece(std::shared_ptr<Square>& square) const
{
